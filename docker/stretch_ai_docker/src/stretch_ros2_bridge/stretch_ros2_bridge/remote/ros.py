...
DEFAULT_COLOR_TOPIC = "/spectacular_ai/color_image"
DEFAULT_DEPTH_TOPIC = "/spectacular_ai/depth_image"
DEFAULT_LIDAR_TOPIC = "/scan_filtered"
...
class StretchRosInterface(Node):
    ...
    def __init__(
        ...
        self.get_logger().error("setup 2")
        self._streaming_activator = StreamingActivator(self)
        self.get_logger().error("Not activating")
    ...
    def send_joint_goals(
        ...
        print(f"sending joint goals {joint_goals}", flush=True)
        with self._js_lock:
    ...
    def _create_services(self):
        ...
        self.pos_mode_service.wait_for_service(1)
    ...
    def _create_pubs_subs(self):
        ...
        self.get_logger().error("creating 21")
        ...
        self.get_logger().error("creating 22 done")
    ...
    def _create_cameras(self, use_d405: bool = True):
        ...
        self.rgb_cam = RosCamera(self, self._color_topic, rotations=STRETCH_HEAD_CAMERA_ROTATIONS, image_ext = "")
    ...
    def _wait_for_cameras(self):
        ...
        self.get_logger().error("Waiting for rgb camera images...")
        self.rgb_cam.wait_for_image()
        self.get_logger().error("Waiting for depth camera images...")
        self.dpt_cam.wait_for_image()
        if self.ee_rgb_cam is not None:
            self.get_logger().error("Waiting for end effector rgb camera images...")
            self.ee_rgb_cam.wait_for_image()
        if self.ee_dpt_cam is not None:
            self.get_logger().error("Waiting for end effector depth camera images...")
            self.ee_dpt_cam.wait_for_image()
        self.get_logger().error("..done.")
        if self.verbose:
            self.get_logger().error(f"rgb frame = {self.rgb_cam.get_frame()}")
            self.get_logger().error(f"dpt frame = {self.dpt_cam.get_frame()}")
    ...
    def _base_state_callback(self, msg: PoseStamped):
        ...
        self.pose_graph_counter += 1
        if self.pose_graph_counter % 120 == 0:
            self.pose_graph_counter = 0
            print("added to pose graph", flush=True)
            p = [
