...
class ZmqServer(BaseZmqServer):
    ...
    def get_state_message(self) -> Dict[str, Any]:
        ...
        if g:
            print(f"step {self._last_step}, at_goal {g}", flush=True)
        return message
    ...
    def handle_action(self, action: Dict[str, Any]):
        ...
        print(f"receive action {action} ################################################################################", flush=True)
        if not (("posture" in action and action["posture"] == "navigation") or "xyt" in action):
            return
