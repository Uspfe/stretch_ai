...
class StretchNavigationClient(AbstractControlModule):
    ...
    def __init__(self, ros_client, robot_model: RobotModel):
        ...
        print("wait for pose", flush=True)
        self._wait_for_pose()
        print("wait for pose ok", flush=True)
    ...
    def move_base_to(
        ...
        """
        self._ros_client.get_logger().info("move base to command")
        # Parse inputs
        ...
        print(f"publihsing goal msg {msg}")
    ...
    def _wait_for_goal_reached(self, verbose: bool = False):
        ...
        while rclpy.ok():
            ...
            recent = self._ros_client.recent_depth_image(self._ros_client.msg_delay_t)
            if verbose:
                print(
            ...
            if at_goal and recent:
                print("at goal!", flush=True)
                break
            else:
                rate.sleep()
